...
.archive-stack { position: relative; min-height: 400px; margin: 1.5rem 0; }
.stack-card {
...
function toggleCard(card) {
  ...
  if (currentActive && currentStack) {
    ...
    if (currentStack !== stack) resetStack(currentStack);
  }
...
function applyShifts(stack, activeCard) {
  ...
  cards.forEach((card, index) => {
    ...
    if (index < activeIndex) {
      ...
      card.style.transform = 'translateY(60px) scale(0.98)';
